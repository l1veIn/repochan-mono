...
5. **`image-edit` has one design-layer user: the `repochan-page-designer` skill.** Post-processing of upstream-generated images (compress / slice / chroma-key / bg-remove / resize / favicon) happens during page assembly and writes derived assets into the scaffolded site's `public/`; derived assets never flow back into `.repochan/`. The starter source (`packages/starters/<id>/`) is read-only — `repochan starter pull` copies it into the output dir (default `.repochan/web-starter/`) and the page-designer edits only the copy. `repochan order slice` / `order extract-stickers` remain available for versioning slices as protocol artifacts, but that is an advanced path, not the page-designer default.
