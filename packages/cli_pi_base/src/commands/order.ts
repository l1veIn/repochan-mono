...
export async function runOrderCommand(cwd: string, args: string[], options: OutputOptions = {}) {
  ...
  if (subcommand === "get") {
    if (!orderId) throw new UsageError("Usage: repochan task get <task-id> [--json]");
    return get(cwd, orderId, options);
  }
  throw new UsageError(`Unknown creation task command: ${subcommand}. Use: repochan task list|get ...`);
}
...
async function get(cwd: string, orderId: string, options: OutputOptions) {
  ...
  heading(`RepoChan creation task ${order.orderId ?? orderId}`);
