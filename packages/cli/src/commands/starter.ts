...
export async function runStarterList(_cwd: string, options: StarterOptions) {
  ...
  const lines = starters.map((starter) => {
    ...
    return `  ${starter.id}  ${dim("—")} ${starter.name}${starter.default ? " (default)" : ""}${tags}`;
  });
...
export async function runStarterConfigure(cwd: string, options: StarterOptions) {
  ...
  const configured = validateStarterSiteConfig(projectStarterSiteConfig({ analysis, persona, defaults }));
...
export async function runStarterAssetApply(cwd: string, slotName: string | undefined, options: StarterOptions) {
  ...
  const result = await readOrderResult(cwd, options.order, options.version);
  ...
  const finalPath = sitePath(target, slot.output);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "repochan-starter-"));
  try {
    if (slot.postprocess?.length) {
      for (const step of slot.postprocess) await applyStep(step, sourceFiles, sitePath(tempRoot, step.out), true);
    } else {
    ...
    const tempFinal = sitePath(tempRoot, slot.output);
    if (!(await exists(tempFinal))) throw new Error(`Postprocess did not produce declared output: ${slot.output}`);
    await fs.rm(finalPath, { recursive: true, force: true });
    await fs.mkdir(path.dirname(finalPath), { recursive: true });
    await fs.cp(tempFinal, finalPath, { recursive: true });
    ...
    emitResult(options, `Applied ${options.order}/${result.version.versionId} → ${slot.output}`, { starter: manifest.id, slot: slot.slot, output: finalPath, orderId: options.order, versionId: result.version.versionId });
  } finally {
...
async function validateStarterDir(cwd: string, starter: StarterMeta): Promise<string[]> {
  ...
  for (const asset of starter.assets) {
    if (asset.order?.templateId && !templateIds.has(asset.order.templateId)) issues.push(`${asset.slot}: unknown templateId ${asset.order.templateId}`);
  }
  try {
