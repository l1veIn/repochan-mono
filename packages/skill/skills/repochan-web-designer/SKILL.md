...
**Cutout 分两类**：通用 cutout 必须完整入画（全身/七分身完整、四边留白），可直接放置在任意区域；出血裁切 cutout 是**设计绑定资产**，仅当页面有 H3/H4 层元素充当接收裁切的视觉边界（卡片缘、section 分界线）时才下单，且不为其他站点复用——starter/通用资产库只收完整版。
