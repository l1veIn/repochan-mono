...
- **人物姿态线稿（pose lineart）**：唯一合法用途是**传递人物姿态**，且仅当人物与页面 H3/H4 层元素存在**结构/空间配合关系**时才需要——例如某个 H4 按钮恰好悬在人物手心上方、人物指向某个具体 section 标题、人物坐在某张卡片边缘。**最典型的场景是出血裁切 cutout**：人物被画布/容器边界裁切（头发扫过 section 分界线、半身探出卡片），这种资产可以进 starter，但必须搭配姿态线稿作为迁移参考——下游复现姿态后才能正确复现裁切关系。此时姿态是页面结构的一部分，线稿是传递它的最佳中介。线稿**只保留人物**：姿态、肢体位置、视线/指向方向、占位轮廓；去掉一切无关信息（场景、道具细节、面部、衣着、渲染风格）。
