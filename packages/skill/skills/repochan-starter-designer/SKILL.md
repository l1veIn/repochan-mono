...
在 manifest 的 `capabilities.sections[]` 固化每个 section 的 composition recipe、baked/live layers、canonical viewport、safe zones、responsive behavior、design provenance 类型、asset slots 与 motion；在 `capabilities.transitions[]` 按页面顺序声明每对相邻 section。将具体 order/version 脱敏为 slot 与迁移要求；保留 fallback，确保 pull 后立即可构建。每个 manifest 必须完整声明 capabilities。

资产 slot 必须显式选择 `kind: "scalar" | "bundle"`：scalar 只声明一个 `output`；bundle 只声明具名 `publications[]`，并使用独占的 `extract-grid` postprocess。对应 `assets.json` 状态使用同一 kind；bundle 只记录 `items`，不伪造代表性顶层 `src`。
...
`starter validate` 负责 manifest、locale、asset contract、required path 与配置结构的确定性检查；source starter build 负责可运行性。浏览器中的响应式、可访问性和视觉一致性仍需按检查表提供人工证据，不得把人工判断伪装成 schema 通过。
...
- `starter validate` 与 build 通过；浏览器检查证据与已知限制被明确记录。
