...
export const assets = {
  ...
  og: { src: bundleSrc("stickers", 0), width: 640, height: 640 },
} as const;

/**
 * 4 桶 theme（primary/base/ink/accents）→ 组件消费的 13 个 CSS 变量
 * （SiteLayout 内联注入）。颜色只在此处由 repochan/site.json 展开，
 * 展示层不出现任何颜色字面量。accents 顺序即下方解构顺序。
 */
const cssTokens = (() => {
  const t = site.theme;
  const [link, linkVisited, buttonFace, win95Teal, selectBlue, chaosRed, chaosGreen, stickyYellow, stickyPink, stickyBlue] =
    t.accents;
  return {
    tape: t.primary,
    paper: t.base,
...
/** <meta name="theme-color">（SiteLayout）—— 与 --win95Teal 同源 */
export const metaThemeColor = cssTokens.win95Teal;
