...
const textures: TextureSlot[] = [
  ...
  {
    src: "/textures/divider.png",
    usage: "divider",
    desc: patterns[1] ?? "Border / divider line texture",
  },
  {
    src: "/textures/sideband.png",
    usage: "sideband",
    desc: patterns[2] ?? "Stripe side decoration",
  },
  {
    src: "/textures/cta-bg.png",
    usage: "cta-bg",
    desc: patterns[3] ?? "Bold color-block CTA backing",
  },
];
...
export function buildCssVars(): string {
  const lines = [
    ...
    `  --texture-divider: url('${textures[1].src}');`,
    `  --texture-sideband: url('${textures[2].src}');`,
    `  --texture-cta: url('${textures[3].src}');`,
