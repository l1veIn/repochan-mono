# Task 09: Page Designer Skill

## 目标

创建 `repochan-page-designer` skill，引导 Agent 执行两阶段页面设计流程：
- Phase 1：设计页面结构 + 资产审计 + 创建缺失的订单
- Phase 2：资产齐全后组装 Page JSON + 渲染

## 文件

- 创建: `packages/pi/skills/repochan-page-designer/SKILL.md`

## 前置

- Task 08 完成（page actions 已接入 pi tool）

## 上下文

现有 skill 的风格（参考 repochan-art-director）：
- frontmatter 有 name + description
- 中文撰写
- 角色定义 → 核心原则 → 执行前检查 → 工作流（分步骤）→ 示例
...
列出演染页面需要的所有图片，对照已有素材，标记缺失项：

```
repochan action="page.check_assets" params={ "page": <你设计的 Page JSON 草稿> }
```

或者先创建一个最小的 Page JSON，然后检查：
```
...
- **og_image**：社交媒体分享预览图（1200×630，必须这个尺寸）
...
SKILL.md 是纯文档，不需要编译。验证方式：

```bash
ls packages/pi/skills/repochan-page-designer/SKILL.md
head -5 packages/pi/skills/repochan-page-designer/SKILL.md
```
...
git add packages/pi/skills/repochan-page-designer/
git commit -m "feat(pi): add repochan-page-designer skill with two-phase workflow"
```
